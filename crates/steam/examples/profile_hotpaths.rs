...
use steam::util::checksum::{Sha1Hash, SteamAdler32};
...
const DEPOT_440_KEY: [u8; 32] = [
    0x44, 0xCE, 0x5C, 0x52, 0x97, 0xA4, 0x15, 0xA1, 0xA6, 0xF6, 0x9C, 0x85, 0x60, 0x37, 0xA5,
    0xA2, 0xFD, 0xD8, 0x2C, 0xD4, 0x74, 0xFA, 0x65, 0x9E, 0xDF, 0xB4, 0xD5, 0x9B, 0x2A, 0xBC,
    0x55, 0xFC,
];
...
fn bench_binary_kv_parsing(iterations: u32) {
    ...
    eprintln!("binary KV size: {} bytes, 500 depots x 10 branches", large_kv.len());
...
fn bench_aes_decrypt(iterations: u32) {
    ...
    for _ in 0..iterations {
        black_box(
            steam::crypto::symmetric_decrypt_ecb(&DEPOT_440_KEY, &zip_chunk).unwrap(),
        );
    }
    let elapsed = start.elapsed();
    let throughput =
        (zip_chunk.len() as f64 * iterations as f64) / elapsed.as_secs_f64() / 1e6;
    eprintln!(
